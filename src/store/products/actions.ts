...
export const addNewProductAction = (product:IProduct, productImgFile:File): TProductAction => async (dispatch, state) => {
    ...
    if(responce.shopUser && responce.data){
        ...
        const img:IFile | null = responce.data;//getLastUploadedSelector(state());
...
export const updateProductAction = (product:IProduct, productImgFile:File): TProductAction => async (dispatch, state) => {

    const userId:string = getUserIdSelector(state());
    let loadedImg:IFile | null = null;
    ...
    const img:IFile | null = loadedImg;
